...
function Dropdown() {
  ...
  return (
    <div className="relative inline-block text-left w-full sm:w-auto">
      <Button
        type='button'
        className="bg-blue-600 text-white px-4 py-2 rounded focus:outline-none dropdown-button w-full sm:w-auto"
        onClick={(e) => handleButtonClick(e)}
      >
        {msgOnBtn}
      </Button>
      {showMenu && (
        <div className="origin-top-right absolute right-0 mt-2 w-full sm:w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none dropdown-menu z-50">
          {options.map((option) => (
            <div
              key={option.id}
              className={`block px-4 py-2 text-sm cursor-pointer ${
