...
const UserInfo = ({ data }) => {
    ...
    const handleSaveClick = async () => {
        try {
            const accessToken = localStorage.getItem("accessToken");
            const response = await axios.patch("https://dsa-tracker-backend-oo1y.onrender.com/users/update-profile", {
    ...
    return (
        <div className="border-2 rounded-md p-4 dark:bg-black relative max-w-full sm:max-w-lg mx-auto">
            {/* Edit Button */}
            {!editMode ? (
                <FaEdit 
                    size="1.5em" 
            ...
            {errorDialog && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 px-4">
                    <div className="bg-white dark:bg-black p-6 rounded shadow-lg w-full max-w-sm">
                        <p className="text-lg text-black dark:text-white">{errorDialog}</p>
                        <button
                            onClick={() => setErrorDialog(null)}
                            className="mt-4 w-full bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded"
